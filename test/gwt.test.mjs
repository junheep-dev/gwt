...
describe("worktree lifecycle", () => {
  ...
  test("removes without asking for trust when no preRemove hook is configured", () => {
    const fixture = createRepository()
    mkdirSync(join(fixture.repository, "scripts"), { recursive: true })
    const hook = join(fixture.repository, "scripts", "setup")
    writeFileSync(hook, "#!/bin/sh\nexit 0\n")
    ...
    git(fixture.repository, "commit", "-m", "Add setup hook")
    assert.equal(gwt(fixture, ["trust"]).status, 0)
    const created = gwt(fixture, ["new", "feature/remove-untrusted"])
    assert.equal(created.status, 0, created.stderr)
    const metadata = JSON.parse(readFileSync(metadataFiles(fixture.repository)[0], "utf8"))
    ...
    const removed = gwt(fixture, ["remove", metadata.id])
    assert.equal(removed.status, 0, removed.stderr)
    assert.doesNotMatch(removed.stderr, /wants to configure/)
    assert.equal(existsSync(metadata.path), false)
  })

  test("requires trust on removal when preRemove is configured", () => {
    const fixture = createRepository()
    mkdirSync(join(fixture.repository, "scripts"), { recursive: true })
    const hook = join(fixture.repository, "scripts", "cleanup")
    writeFileSync(hook, "#!/bin/sh\nprintf 'cleaned\\n'\n")
    chmodSync(hook, 0o755)
    writeConfig(fixture.repository, { preRemove: "scripts/cleanup" })
    git(fixture.repository, "add", "scripts/cleanup")
    git(fixture.repository, "commit", "-m", "Add cleanup hook")
    assert.equal(gwt(fixture, ["trust"]).status, 0)
    const created = gwt(fixture, ["new", "feature/remove-hook"])
    assert.equal(created.status, 0, created.stderr)
    const metadata = JSON.parse(readFileSync(metadataFiles(fixture.repository)[0], "utf8"))

    assert.equal(gwt(fixture, ["trust", "--revoke"]).status, 0)
    const untrusted = gwt(fixture, ["remove", metadata.id])
    assert.equal(untrusted.status, 1)
    assert.match(untrusted.stderr, /Project configuration is not trusted/)
    assert.equal(existsSync(metadata.path), true)

    assert.equal(gwt(fixture, ["trust"]).status, 0)
    const removed = gwt(fixture, ["remove", metadata.id])
    assert.equal(removed.status, 0, removed.stderr)
    assert.match(removed.stdout, /cleaned/)
    assert.equal(existsSync(metadata.path), false)
  })
